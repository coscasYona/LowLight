...
def _resolve_raw_path(base_dir, candidate, fallback_subdir):
    """Return an absolute path for RAW files regardless of how the list encodes it."""
...
class SIDDataset(BaseDataset):
    ...
    def __getitem__(self, i):
        ...
        input_path = _resolve_raw_path(self.datadir, input_fn, 'short')
        target_path = _resolve_raw_path(self.datadir, target_fn, 'long')

        ratio = compute_expo_ratio(input_fn, target_fn)
        iso_input, _ = metainfo(input_path)
        CRF = self.CRF         
