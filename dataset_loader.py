...
class SID_Dataset_Denoise_raw(Dataset):
    def __init__(self, dataset_dir, patchsize=None, train_flag = True):
        ...
        input_path = os.path.join(dataset_dir, input_dir)

        if not os.path.isdir(gt_path) or not os.path.isdir(input_path):
            raise FileNotFoundError(
                f"Expected MAT dataset folders '{gt_dir}' and '{input_dir}' inside '{dataset_dir}'. "
                "If you intend to train from raw SID short/long pairs, pass '--use_sid_raw' with the "
                "correct '--trainset_path' and '--train_list'."
            )

        clean_files = sorted(os.listdir(gt_path))
        noisy_files = sorted(os.listdir(input_path))
        ...
        self.clean_filenames = [os.path.join(gt_path, x) for x in clean_files if is_mat_file(x)]
        self.noisy_filenames = [os.path.join(input_path, x) for x in noisy_files if is_mat_file(x)]
